...
var BASEURL = "http://api.brela.go.tz/";
...
//     const token = req.headers['authorization'] || req.query.token; // Check Authorization header and query parameter
...
//     // Split 'Bearer <token>' and get the actual token if using Authorization header
//     const actualToken = token.startsWith('Bearer ') ? token.split(' ')[1] : token; // Adjust based on source
...
//         // Store the decoded token data in req.user
//         req.user = decoded;

//         // Optionally assign userID from the decoded token (if it exists) to the session
//         if (decoded.uid) {
  ...
  request({
    ...
    request({
      ...
      request({
        ...
        method: 'POST',
        json: {
          OldTrackingNo: OldTrackingNo, BusinessLicenceApplicationId: BusinessLicenceApplicationId,
                          ...
                          function (err5, recordset) {
                            ...
                            request4.query('SELECT Id, Email, PhoneNo, PoBox FROM dbo.BusinessLicenceDetails WHERE BusinessLicenceApplicationId = @BusLicAppId',
                              function (err6, recordset6) {
                                ...
                                sql.close();
                                res.send({
                                  "status": "success", "newBLAppId": BLAppId, "Id_bl": Id_bl, "Email": Email, "PhoneNo": PhoneNo, "PoBox": PoBox,
    ...
    request({
      ...
      request({
        ...
        request({
          ...
          method: 'POST',
          json: {
            OldTrackingNo: OldTrackingNo, newtrackingNo: newtrackingNo, BLdetailsPoBox: BLdetailsPoBox,
  ...
  if (typeof req.session.userID !== "undefined" || req.session.userID === true) {
    ...
    request({
      ...
      for (var i = 0; i < jsonData.length; i++) {
        ...
        objs12.push({
          "Id": Id, "BusinessLicenceClassCode": BusinessLicenceClassCode,
...
app.post('/SubmitSuppliment', function (req, res) {
  ...
  sql.connect(configBL, function (err) {
    ...
    request1.query('SELECT Id FROM dbo.BusinessLicApplication where TrackingNo = @trackingNo',
      function (err, recordset) {
        if (err) {
          console.log("fail to BusinessLicApplicationi " + err);
          sql.close();
          res.send({ status: "failed" });
        }
        var result_from = recordset.recordset;
                                                                  ...
                                                                  request({
                                                                    ...
                                                                    if (error) {
                                                                      ...
                                                                      if (body !== undefined) {
                                                                        ...
                                                                        objs13.push({
                                                                          "AmountTotal": AmountTotal, "ServiceName": ServiceName, "ExpireDate": ExpireDate,
                                                                        ...
                                                                        request({
                                                                          ...
                                                                          if (error) {
                                                                            ...
                                                                            for (var i = 0; i < jsonData.length; i++) {
                                                                              ...
                                                                              objs14.push({
                                                                                "InvoiceNo": InvoiceNo, "ItemName": ItemName, "Amount": Amount,
                                                                            ...
                                                                            request({
                                                                              ...
                                                                              if (error) {
                                                                                ...
                                                                                if (body !== undefined) {
                                                                                  ...
                                                                                  res.send({
                                                                                    "payment": objs13, "WardName": WardName, "AplicationID": AplicationID, "ServiceCode": ServiceCode,
    ...
    request({
      ...
      if (error) {
        ...
        for (var i = 0; i < jsonData.length; i++) {
          ...
          objs12.push({
            "status": "success", "OwnerSubTypeId": OwnerSubTypeId,
...
app.get('/GetBLAttachment', function (req, res) {
  ...
  if (typeof req.session.userID !== "undefined" || req.session.userID === true) {
    request({
      ...
      console.log("BizOwType =" + req.session.BizOwnerType);
      console.log("Id =", response.body);
      var jsonData = JSON.parse(response.body);
      for (var i = 0; i < jsonData.length; i++) {

        console.log(jsonData[i].AttachmentTypeId);
        console.log(jsonData[i].AttachmentName);
        console.log(jsonData[i].Description);
        console.log(jsonData[i].Path);
...
app.get('/GetBusTypePermits/:id', function (req, res) {
  ...
  console.log('....nmefika................')
  console.log('BizNumber is: ', bizNo)
  var objs12 = [];
  console.log(bizNo)
  if (typeof req.session.userID !== "undefined" || req.session.userID === true) {
    request({
      ...
      for (var i = 0; i < jsonData.length; i++) {
        ...
        var PermitPath = jsonData[i].PermitPath;
...
//   var attachmentId = req.body.attachmentId;
//   var trackNo = req.body.trackNo; // Will be updated with new trackingNo
//   var userId = req.session.userID;
//   var token = req.session.token;
//   let atachment = req.body.atachment;

//   // Clean up the attachment to remove the data URL header
//   let attachment = atachment.replace("data:application/pdf;base64,", "");

//   // Check if the user is authenticated
//   if (req.session.userID) {

//     // Step 1: Generate the tracking number using the TrackNoGen endpoint
//     request({
//       url: TrackNoGen,
//       method: 'GET',
//     }, function (error, response, body) {
//       if (error) {
//         console.log("Failed to generate tracking No: " + error);
//         return res.send({ status: "failed" });
//       }

//       // Parse the response to get the new tracking number
//       var jsonData = JSON.parse(response.body);
//       console.log('response: ', response.body)
//       var newtrackingNo = jsonData[0].trackingNo; // Assuming the structure from your example

//       // Step 2: Upload the attachment using the newly generated tracking number
//       request({
//         url: UploadFxPLink,
//         method: 'POST',
//         json: {
//           trackNo: trackNo, // Use the new tracking number
//           trackngNo: newtrackingNo, // Assuming this is a different session-based trackingNo
//           userId: userId,
//           attachmentId: attachmentId,
//           atachment: attachment,
//           token: token
...
//       }, function (error1, response1, body1) {
//         if (error1) {
//           console.log("Failed to upload FxP: " + error1);
//           return res.send({ status: "failed" });
//         }

//         console.log("Upload response body: ", response1.body);

//         // Send a success response back to the client
//         res.send({ status: "success", response: response1.body });
//       });
//     });
...
app.post('/NINVerification', function (req, res) {
  ...
  // Ensure NIN is at least 8 characters long before slicing
  if (ninNo.length < 8) {
    return res.send({ "status": "failed", "message": "Invalid NIN format" });
  }

  // Extract year, month, and day from NIN
  var year = ninNo.slice(0, 4);
  var month = ninNo.slice(4, 6);
  var day = ninNo.slice(6, 8);
  var ninDate = year + '-' + month + '-' + day;

  console.log(`Verifying NIN: ${ninNo} with DOB: ${ninDate}`);
  ...
  if (typeof req.session.userID !== "undefined" && req.session.userID === true) {
    request({
      ...
      if (error) {
        console.log("Failed to verify NIN: " + error);
        return res.send({ "status": "failed", "message": "Verification error" });
      }

      console.log(response.body);

      // Check for response type errors
      if (response.body === 'TypeError') {
        return res.send({ "status": "failed", "message": "Invalid response from NIN API" });
      }
      ...
      try {
        ...
        if (resultcode == 0) {
          ...
          // Handle cases where resultcode isn't 0
          res.send({ "status": "failed", "message": "NIN verification failed" });
        }

      } catch (parseError) {
        console.log("JSON parse error: " + parseError);
        res.send({ "status": "failed", "message": "Invalid JSON response" });
      }
    });
  } else {
...
app.post('/saveStageSecond', function (req, res) {
  ...
  if (typeof req.session.userID !== "undefined" || req.session.userID === true) {
    request({
      ...
      method: 'POST',
      json: {
        fullname: fullname, bn_no: bn_no, bn_name: bn_name, company_name: company_name, inco_no: inco_no,
      ...
      request({
        ...
        method: 'POST',
        json: {
          fullname: fullname, bn_no: bn_no, bn_name: bn_name,
...
app.post('/updateStageSecond', function (req, res) {
  ...
  if (typeof req.session.userID !== "undefined" || req.session.userID === true) {
    ...
    request({
      ...
      method: 'POST',
      json: {
        districtlistB: districtlistB, regionlistB: regionlistB,
  ...
  if (typeof req.session.userID !== "undefined" || req.session.userID === true) {
    request({
      ...
      for (var i = 0; i < jsonData.length; i++) {
        ...
        objs12.push({
          "BusinessClassId": BusinessClassId, "EntityName": EntityName,
  ...
  if (typeof req.session.userID !== "undefined" || req.session.userID === true) {
    request({
      ...
      for (var i = 0; i < jsonData.length; i++) {
        ...
        objs12.push({
          "EntityName": EntityName, "LicenceStatusId": LicenceStatusId, "IsSentRegistry": IsSentRegistry, "ServiceCode": ServiceCode,
  ...
  sql.connect(configBL, function (err) {
    ...
    request.query('SELECT d.FeePaid as FeePaid, a.BusinessClassId as BusinessClassId, a.EntityName as EntityName, d.LicenceStatusId as LicenceStatusId, d.IsBranch as IsBranch, d.BLNumber as BLNumber, a.Id as Id, a.TrackingNo as TrackingNo, a.ServiceCode as ServiceCode, d.ExpireDate as ExpireDate, d.DateIssued as DateIssued, b.ApplicationStatusId as ApplicationStatusId, b.ApplicationStageId as ApplicationStageId, c.BusinessTypeName as BusinessTypeName, b.PaymentStatus as PaymentStatus, d.isSentToRegistry as IsSentRegistry FROM dbo.BusinessLicApplication as a, dbo.BLicenseApplicationTracker as b, dbo.BusinessTypes as c, dbo.BusinessLicenceDetails as d WHERE d.BusinessLicenceApplicationId = a.Id AND a.Id = b.ApplicationId AND c.BusinessTypeId = a.BusinessTypeId AND b.ApplicationStatusId = 5 AND d.LicenceStatusId NOT IN (1,2,3) AND d.isSentToRegistry = 1 AND b.ApplicationStageId = 6 AND a.CreatedByUserId = ' + userId + ' ORDER BY a.Id DESC',
      function (err, recordset) {
        ...
        if (err) {
          console.log("fail to MyLicCan " + err);
          //sql.close();
          res.send({ status: "failed" });
        }
        var result_from = recordset.recordset;
...
app.post('/recordAddressInfo', function (req, res) {
  ...
  sql.connect(config, function (err) {
    ...
    request1.query('SELECT max(Id) as Id FROM dbo.tblAddress', function (err, recordset) {
      if (err) {
        console.log("fail to recordAddressInfo " + err);
        // sql.close();
        res.send({ status: "failed" });
      }
      // sql.close();
...
app.post('/submitApplication', function (req, res) {
  ...
  console.log('tracking no: ', trackngNo)
...
app.post('BStageTwo', function (req, res) {
  //console.log(req.body)
  var trackngNo = req.body.trackngNo;
  var BLNo = req.body.BLNo;
  var trackNo = req.body.trackNo;
  ...
  sql.connect(configBL, function (err) {
    ...
    if (err) {
      ...
      request1.query('SELECT Id FROM dbo.BusinessLicApplication WHERE TrackingNo = @trackngNo',
        function (err, recordset) {
          if (err) {
            console.log("fail to uploaadFiley " + err);
            //sql.close();
            res.send({ status: "failed" });
          } else {
            var result_form = recordset.recordset;
            ...
            request({
              ...
              if (error) {
                console.log("fail to addBLAttachment " + error);
                //sql.close();
                res.send({ "failed": "failed" });
              } else {
                res.send({ "sucess": "sucess" })
    ...
    if (err) {
      ...
      request.query('SELECT * FROM dbo.BusinessTypes WHERE BusinessCategoryId = @reg_id',
        function (err, recordset) {
          ...
          if (err) {
            console.log("fail to bizType " + err);
            //sql.close();
            res.send({ status: "failed" });
          } else {
            sql.close();
    ...
    if (err) {
      ...
      request.query('SELECT * FROM dbo.BusinessTypes',
        function (err, recordset) {
          ...
          if (err) {
            console.log("fail to bizTypeAll " + err);
            //sql.close();
            res.send({ status: "failed" });
          } else {
            sql.close();
    ...
    if (err) {
      ...
      request.query('SELECT * FROM dbo.tblDistrict WHERE RegionCode = @reg_id',
        function (err, recordset) {
          ...
          if (err) {
            console.log("fail to regions " + err);
            //sql.close();
            res.send({ status: "failed" });
          } else {
            sql.close();
    ...
    if (err) {
      ...
      request.query('SELECT DistrictName FROM dbo.tblDistrict WHERE DistrictCode = @reg_id',
        function (err, recordset) {
          ...
          if (err) {
            console.log("fail to GetSavedDistrict " + err);
            //sql.close();
            res.send({ status: "failed" });
          } else {
            sql.close();
  ...
  sql.connect(configBL, function (err) {
    if (err) {
      ...
      request1.query('SELECT Id FROM dbo.BusinessLicApplication WHERE TrackingNo = @trackngNo',
        function (err, recordset) {
          if (err) {
            console.log("fail to atachFile " + err);
            sql.close();
            res.send({ status: "failed" });
          }
          // sql.close();
          ...
          request.query(`INSERT INTO dbo.BusinessLicenceAttachments (Path, AttachmentTypeId, BusinessLicenceApplicationid, TrackingNo, Name, IsPermit) values (@attachmentPath, @categoryID, @req_id, @trackNo, @trackNo, @uploadType)`,
            function (err, recordset) {
              if (err) {
                console.log("fail to BusinessLicenceAttachments " + err);
                //sql.close();
                res.send({ status: "failed" });
              } else {
                sql.close();
    ...
    if (err) {
      ...
      request.query('SELECT * FROM dbo.BusinessCategory WHERE IsDeleted = 0 and BusinessSectorId = @reg_id',
        function (err, recordset) {
          ...
          if (err) {
            console.log("fail to category " + err);
            // sql.close();
            res.send({ status: "failed" });
          } else {
            sql.close();
    ...
    if (err) {
      ...
      request.query('SELECT * FROM dbo.tblWard WHERE DistrictCode = @reg_id',
        function (err, recordset) {
          ...
          if (err) {
            console.log("fail to district " + err);
            // sql.close();
            res.send({ status: "failed" });
          } else {
            sql.close();
    ...
    if (err) {
      ...
      request.query('SELECT * FROM dbo.tblWard WHERE WardCode = @reg_id',
        function (err, recordset) {
          ...
          if (err) {
            console.log("fail to postcode " + err);
            //sql.close();
            res.send({ status: "failed" });
          }
          sql.close();
    ...
    if (err) {
      ...
      request.query('DELETE FROM BusinessLicApplication WHERE TrackingNo = @TrackingNo',
        function (err, recordset) {
          ...
          if (err) {
            console.log("fail to dltPendApp " + err);
            //sql.close();
            res.send({ "status": "failed" });
          } else {
        ...
        function (err1, recordset) {
          ...
          if (err1) {
            ...
            if (jsonData.length <= 0) {
              ...
              var InvoiceID = "";
              obj.push({
                'AmountTotal': AmountTotal, 'ServiceName': ServiceName,
    ...
    if (err) {
      ...
      request.query('SELECT * from dbo.tblAddress as a, dbo.tblAreaType as b ' +
        ...
        function (err, recordset1) {
          ...
          if (err) {
            console.log("fail to getSavedAreaType " + err);
            // sql.close();
            res.send({ status: "failed" });
          } else {
            sql.close();
    ...
    if (err) {
      ...
      request.query('SELECT * from dbo.tblOwnerSubType where OwnerSubTypeId = @BizOwnerTypeId',
        function (err, recordset1) {
          ...
          if (err) {
            console.log("fail to GetbusOnwerTypeName " + err);
            //sql.close();
            res.send({ status: "failed" });
          } else {
            sql.close();
    ...
    if (err) {
      ...
      request.query('SELECT ApplicationStatusName from dbo.ApplicationStatus WHERE Id = @ApplicationStatusId',
        function (err, recordset1) {
          ...
          if (err) {
            console.log("fail to GetApplicationStatus " + err);
            // sql.close();
            res.send({ status: "failed" });
          } else {
  ...
  sql.connect(config, function (err) {
    ...
    if (err) {
      ...
      request.query('SELECT RegionName FROM dbo.tblRegion WHERE RegionCode = @regionCode', function (err, recordset) {
        ...
        if (err) {
          console.log("fail to getSavedRegion " + err);
          // sql.close();
          res.send({ status: "failed" });
        } else {
          sql.close();
    ...
    if (err) {
      ...
      request.query('SELECT * from BusinessSector',
        function (err, recordset) {
          ...
          if (err) {
            console.log("fail to sectors " + err);
            // sql.close();
            res.send({ status: "failed" });
          } else {
            sql.close();

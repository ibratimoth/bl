	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       
		    ...
		    if(quantity[x].value == ''){	    	
		    	$("#"+quantityId+"").after('<p class="text-danger"> QuantityField is required!! </p>');
	   	...
	   	var validateDelivery;
	   	for (var x = 0; x < delivery.length; x++) {       
	 			var deliveryId = delivery[x].id;
		    if(delivery[x].value == ''){	    	
		    	$("#"+deliveryId+"").after('<p class="text-danger"> Delivery Field is required!! </p>');
		    	$("#"+deliveryId+"").closest('.form-group').addClass('has-error');	    		    		    	
	      } else {      	
		    	$("#"+deliveryId+"").closest('.form-group').addClass('has-success');	    		    		    		    	
	      } 
	   	}  // for
	   	...
	   	for (var x = 0; x < delivery.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true && validateDelivery == true) {
					// create order button
	...
	$.ajax({
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'<td style="padding-left:20px;">'+
				'<div class="form-group">'+
				'<input type="number" name="delivery[]" id="delivery'+count+'" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control"  />'+
				'</div>'+
			'</td>'+
				'<td style="padding-left:20px;">'+
